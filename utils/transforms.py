...
def get_transforms(mode, dataset, multicrop=False, num_large_crops=2, num_small_crops=2, is_swav=False):

    if "CIFAR100Split_" in dataset:
        dataset = "CIFAR100"  # It's the same data, just different labels
    ...
    transform = transform_dict[dataset][mode]

    if mode == "unsupervised" or is_swav:
        ...
        if is_swav:
            transforms += [transform_dict[dataset]["eval"]]
