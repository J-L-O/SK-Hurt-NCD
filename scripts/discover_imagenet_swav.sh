#!/usr/bin/env bash
# This script is used to discover novel classes in our ImageNet-based benchmark based on SwAV pretraining.
...
python main_discover.py \
  ...
  --num_heads 4 \
  --pretrained "$CHECKPOINT_PATH"/pretrain-resnet18-ImageNet-entity30_swav_"$labeled_set""$unlabeled_set".cp \
  --imagenet_subset entity30 \
  --imagenet_split "$labeled_set""$unlabeled_set" \
  --comment entity30_swav_"$labeled_set""$unlabeled_set"_alpha"$alpha" \
